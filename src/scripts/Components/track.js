import formatStastCount from "../Utils/formatStatsCount";
...
export const Track = (track) => {
  ...
  <div  class="track border border-dark text-dark bg-primary mb-3">
            ...
            <div data-stream="${id}" data-title="${title}" class="card-body d-flex flex-column justify-content-between">
                ...
                <i aria-label="Playback Count" title="Playbacks ${playback_count}" class="fas fa-play"></i> ${formatStastCount(
    playback_count
  )}
                <i aria-label="Favorites Count" title="Likes ${favoritings_count}" class="fas fa-heart"></i> ${formatStastCount(
    favoritings_count
  )}
            </div> 
